...
export class AddendumDetailComponent implements OnInit {
  contractItemDialogRef: MatDialogRef<AddendumItemComponent>;
  ...
  constructor(private _formBuilder: FormBuilder,
    ...
    this.form = this._formBuilder.group({
      ...
      contract_ver: 0,
      latest_flag: true,
      addendum_type_id: [{ value: '', disabled: disabledControl }, Validators.required],
      addendum_no: [{ value: '', disabled: true }],
      addendum_date: [{ value: null, disabled: disabledControl }, Validators.required],
      ...
      shipment_to_id: [{ value: '', disabled: true }],
  ...
  ngOnInit() {
    ...
    //mock id
    //this.id = "1";
    this.form.get('addendum_type_id')
      .valueChanges
      .subscribe((res) => {
        if (res) {
          let addendum_type = _.find(this.addendum_types, x => x.id == res);
          if (addendum_type) {
            const controls = this.form.controls;
            if (addendum_type.lov_code == "4") {
              for (const name in controls) {
                if (name.substr(0, 3) == 'ad_') {
                  controls[name].enable({ emitEvent: false });
                }
              }
              this.form.get('ad_shipment_term').disable({ emitEvent: false });
              if (!this.finalFlag) {
                this.disableAddProduct = false;
              }

            } else if (addendum_type.lov_code == "3" || addendum_type.lov_code == "2") {
              for (const name in controls) {
                if (name.substr(0, 3) == 'ad_') {
                  controls[name].enable({ emitEvent: false });
                }
    ...
    this.form.get('shipment_term_id')
      .valueChanges
      .subscribe((res) => {
        this.changeShipmentTerm(res)
      });


    this.form.get('ad_buyer_contract_no')
      .valueChanges
      .subscribe((res) => {
        if (res) {
          this.form.get('buyer_contract_no').enable();
        } else {
          this.form.get('buyer_contract_no').disable();
          this.form.get('buyer_contract_no').setValue(this.contract.buyer_contract_no);
        }
      });

    this.form.get('ad_buyer')
      .valueChanges
      .subscribe((res) => {
        if (res) {
          this.form.get('buyer_id').enable();
        } else {
          this.form.get('buyer_id').disable();
          this.form.get('buyer_id').setValue(this.contract.buyer_id);
        }
      });

    this.form.get('ad_shipment_term')
      .valueChanges
      .subscribe((res) => {
        if (res) {
          this.form.get('shipment_term_id').enable();
        } else {
          this.form.get('shipment_term_id').disable();
          this.form.get('shipment_term_id').setValue(this.contract.shipment_term_id);
        }
      });

    this.form.get('ad_shipment_option')
      .valueChanges
      .subscribe((res) => {
        if (res) {
          ...
          this.form.get('shipment_option').setValue(this.contract.shipment_option);
          this.form.get('shipment_to_id').setValue(this.contract.shipment_to_id);
          this.form.get('gen_term_condition_id').setValue(this.contract.shipment_to_id);
        }
      });

    this.form.get('ad_partial_shipment_opt')
      .valueChanges
      .subscribe((res) => {
        if (res) {
          this.form.get('partial_shipment_option_id').enable({ emitEvent: false });
        } else {
          this.form.get('partial_shipment_option_id').disable();
          this.form.get('partial_shipment_option_id').setValue(this.contract.partial_shipment_option_id);
          this.form.get('gen_term_condition_id').disable();
          this.form.get('gen_term_condition_id').setValue(this.contract.gen_term_condition_id);
          this.form.get('gen_term_condition_desc').setValue(false);
        }
      });
    this.form.get('ad_shipment_to')
      .valueChanges
      .subscribe((res) => {
        if (res) {
    ...
    this.form.get('ad_exchange_rate')
      .valueChanges
      .subscribe((res) => {
        if (res) {
          this.form.get('exchange_rate').enable();
        } else {
          this.form.get('exchange_rate').disable();
          this.form.get('exchange_rate').setValue(this.contract.exchange_rate);
        }
      });
    this.form.get('ad_shipment_term_remark')
      .valueChanges
      .subscribe((res) => {
        if (res) {
          this.form.get('shipment_term_remark').enable();
        } else {
          this.form.get('shipment_term_remark').disable();
          this.form.get('shipment_term_remark').setValue(this.contract.shipment_term_remark);
        }
      });
    this.form.get('ad_product_remark')
      .valueChanges
      .subscribe((res) => {
        if (res) {
          this.form.get('product_remark').enable();
        } else {
          this.form.get('product_remark').disable();
          this.form.get('product_remark').setValue(this.contract.product_remark);
        }
      });
    this.form.get('ad_gen_term_condition')
      .valueChanges
      .subscribe((res) => {
        if (res) {
    ...
    combineLatest(
      ...
      //LOV
      this.addendum_types = _.sortBy(_.filter(this.allLovs, x => x.lov_group.toUpperCase() == 'SYSTEM' && x.lov_type.toUpperCase() == 'ADDENDUM TYPE' && x.record_status), "lov_order");
      ...
      if (this.id) {
        this.odata.Get(this.id)
          .Expand('contract_items($expand=product,pu_code,unit_name_eng,pu_sub_code,price_type),addendum_type,partial_shipment_option')
          .Exec()
          .subscribe(contract => {
            ...
            this.contract_items = _.sortBy(_.filter(this.allContract_items, x => x.record_status == true), "product.name_en");
            ...
            this.form.patchValue({
              ...
              contract_ver: this.contract.contract_ver,
              addendum_type_id: this.contract.addendum_type_id,
              addendum_no: this.contract.addendum_no,
              addendum_date: this.contract.addendum_date,
              ...
              ad_buyer_contract_no: this.contract.ad_buyer_contract_no,
              ad_buyer: this.contract.ad_buyer,
              ad_shipment_term: this.contract.ad_shipment_term,
              ad_shipment_option: this.contract.ad_shipment_option,
              ad_partial_shipment_opt: this.contract.ad_partial_shipment_opt,
              ad_shipment_to: this.contract.ad_shipment_to,
              ad_payment_term: this.contract.ad_payment_term,
              ad_exchange_rate: this.contract.ad_exchange_rate,
              ad_shipment_term_remark: this.contract.ad_shipment_term_remark,
              ad_product_remark: this.contract.ad_product_remark,
              ad_gen_term_condition: this.contract.ad_gen_term_condition,
            ...
            if (this.mode == "View") {
              ...
              this.form.get('addendum_type_id').disable();
              //this.form.get('addendum_date').enable();
            }
        ...
        this.odata.Get(this.ref)
          ...
          .subscribe(contract => {
            ...
            this.form.patchValue({
              ...
              addendum_no: this.contract.addendum_no ? (this.contract.addendum_no + 1) : 1,
              addendum_date: null,
              contract_status: this.contract.contract_status,
              contract_made_on_id: this.contract.contract_made_on_id,
              crop_year: this.contract.crop_year,
              ...
              total_qty: this.contract.total_qty,
              total_shipment: this.contract.total_shipment,
              ad_buyer_contract_no: false,
              ad_buyer: false,
              ad_shipment_term: false,
              ad_shipment_option: false,
              ad_partial_shipment_opt: false,
              ad_shipment_to: false,
              ad_payment_term: false,
              ad_exchange_rate: false,
              ad_shipment_term_remark: false,
              ad_product_remark: false,
              ad_gen_term_condition: false,
              record_status: true,
              created_by_id: this.user.employee_username,
              created_date: moment(),
              updated_by_id: null,
              updated_date: null,
              ref_contract_no: this.contract.contract_no,
              //ref_contract_id: this.ref,
              // ref_contract: this.contract.ref_contract,
              pricing_method: this.contract.pricing_method,
            }, { emitEvent: false });
            ...
            this.form.get('addendum_type_id').enable();
            this.form.get('addendum_date').enable();
  ...
  getAddendumValue(id){
    if (id){
      let val = _.find(this.addendum_types,x=>x.id ==id);
      if (val){
        return val.lov_code;
      }
    }
  ...
  getFlag() {
    this.odata
      .Query()
      .Filter("ref_contract_no eq '" + this.contract.contract_no + "' and contract_type eq 'ADDENDUM' and record_status eq true")
      .Expand('addendum_type')
      .OrderBy('addendum_no')
      .Exec()
      .subscribe(addendums => {
        addendums = _.filter(addendums, x => x.id != this.id);
        this.maxType = _.maxBy(addendums, "addendum_type.lov_code");
        let finalList = _.filter(addendums, x => x.addendum_type.lov_code == "2");
        let proList = _.filter(addendums, x => x.addendum_type.lov_code == "2");
        if (finalList.length) {
          this.finalFlag = true;
        }
        if (proList.length) {
        ...
        this.addendum_types = _.filter(this.addendum_types, x => x.lov_code >= this.maxType.addendum_type.lov_code);
      });
  }
  ...
  changePartialShipmentOption(val) {
    ...
    if (partial_shipment_option) {
      ...
      this.form.get('ad_gen_term_condition').setValue(true);
      this.form.get('gen_term_condition_id').enable();

      if (partial_shipment_option.lov_code.toUpperCase() == 'EPZ') {
        if (this.contract.shipment_to_id == this.form.get('shipment_to_id').value) {
          this.form.get('ad_shipment_to').setValue(false, { emitEvent: false });
          this.form.get('shipment_to_id').disable({ emitEvent: false });
          this.form.get('shipment_to_id').setValue(this.contract.shipment_to_id, { emitEvent: false });
        } else {
          this.form.get('ad_shipment_to').setValue(true, { emitEvent: false });
          this.form.get('ad_shipment_to').enable({ emitEvent: false });
          this.form.get('shipment_to_id').enable({ emitEvent: false });
          this.form.get('shipment_to_id').setValue(null, { emitEvent: false });
        }
      } else {
        if (this.contract.shipment_to_id == this.form.get('shipment_to_id').value) {
          this.form.get('ad_shipment_to').setValue(false);
          this.form.get('ad_shipment_to').disable({ emitEvent: false });
          this.form.get('shipment_to_id').disable({ emitEvent: false });
  ...
  ClearInsuranceCost() {
    for (let contract_item of this.allContract_items) {
      if (contract_item.record_status == true) {
        contract_item.insurance_cost = 0;
        contract_item.freight_cost = 0;
      }
    }
    this.calNetPrice();
  }
  ...
  calNetPrice() {
    for (let contract_item of this.allContract_items) {
      contract_item.net_price = contract_item.unt_price + contract_item.insurance_cost + contract_item.freight_cost;
    }
    this.contract_items = _.sortBy(_.filter(this.allContract_items, x => x.record_status == true), "product.name_en");
  }

  changeShipmentTerm(val) {
    let shipment_term = _.find(this.shipment_terms, x => x.id == val);
    if (shipment_term) {
      switch (shipment_term.shipterm_code) {
        case "FOB": {
          this.ClearInsuranceCost();
          this.ClearFreightCost()
          break;
        }
        case "CIF": {
      ...
      this._matSnackBar.open('Change shipment term maybe effect contract item.', 'Warning', {
        verticalPosition: 'top',
        duration: 10000
      });
    }
  ...
  changeGenTermCondition(val) {
    let term_cond = _.find(this.AllTerm_conds, x => x.id == val);
    if (term_cond) {
      this.form.patchValue({
        gen_term_condition_desc: term_cond.tc_in_contract
      });
    } else {
      this.form.patchValue({
        gen_term_condition_desc: ""
  ...
  //openAddendumItemDialog 
  openAddendumItemDialog(contract_item: contract_item, mode: string) {
    ...
    let selectedAddendumType = _.find(this.addendum_types, x => x.id == this.form.controls['addendum_type_id'].value);

    this.contractItemDialogRef = this.dialog.open(AddendumItemComponent, {
      height: '80%',
      width: '60%',
      data: {
        ...
        addendum_type: selectedAddendumType,
        contract_items: this.contract_items
      }
    });
    ...
    this.contractItemDialogRef.afterClosed().pipe(
      ...
      if (ret) {
        if (ret.id) {
          ...
          this._matSnackBar.open('Addendum item saved', 'OK', {
            verticalPosition: 'top',
            duration: 10000
          });
        } else {
          ...
          this._matSnackBar.open('Addendum item added', 'OK', {
            verticalPosition: 'top',
            duration: 10000
          });
  ...
  deleteAddendumItem(id) {
    let contract_item = _.find(this.allContract_items, x => x.id == id);
    if (contract_item) {
      if (contract_item.shipment_qty > 0) {
        this._matSnackBar.open('Addendum item cannot delete : Shipment Qty > 0', 'Error', {
          verticalPosition: 'top',
          duration: 10000
        });
      } else {
        ...
        // this.form.get('gen_term_condition_id').enable();
        // }
        this._matSnackBar.open('Addendum item deleted', 'OK', {
          verticalPosition: 'top',
          duration: 10000
        });
      }
  ...
  checkPriceType(addendum_type) {
    let i = 1;
    let contract_items = _.sortBy(this.contract_items, "product.name_en");
    for (let contract_item of contract_items) {
      if (contract_item.price_type.lov_code < addendum_type) {
        this.errors.push( i);
      }
      i++;
    }
  ...
  validate(data) {
    this.errors = [];
    if (data.addendum_type_id) {
      let addendum_type = _.find(this.addendum_types, x => x.id == data.addendum_type_id)
      switch (addendum_type.lov_code) {
        case "2": {
          this.checkPriceType("2")
          break;
        }
        case "3": {
          this.checkPriceType("3")
          break;
        }
        case "4": {
          return true
    ...
    if (this.errors.length) {
      this._matSnackBar.open('Cannot save. Contract item ' + this.errors.join(', ') + ': Price type invalid.', 'ERROR', {
        verticalPosition: 'top',
        duration: 10000
      });
  ...
  addAddendum() {
    ...
    this.odata.Post(
      ...
      .subscribe(
        resolve => {
          this._matSnackBar.open('Addendum no.' + data.addendum_no + ' added', 'OK', {
            verticalPosition: 'top',
            duration: 10000
          });
      ...
      .subscribe((old: contract) => {
        if (!old.updated_date || moment(old.updated_date).toDate().getTime() == moment(data.updated_date).toDate().getTime()) {
          ...
          this.odata.Put(
            ...
            .subscribe(
              resolve => {
                this._matSnackBar.open('Addendum no.' + data.contract_no + ' saved', 'OK', {
                  verticalPosition: 'top',
                  duration: 10000
                });
                // Change the location with new one
          ...
          this._matSnackBar.open("Cannot save. Addendum has been edit by " + old.updated_by_id, 'ERROR', {
            verticalPosition: 'top',
            duration: 10000
          });
        }
